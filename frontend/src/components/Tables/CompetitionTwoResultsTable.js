...
import CompetitionTwoResultsAdminDisplay from '../Displays/CompetitionTwoResultsAdminDisplay.js'
...
import {clientsDetailsResultsForCompetitionTwo} from '../../utils/utils'
...
const CompetitionTwoResultsTable = (props) =>{
  ...
  // ii) Lifting state variables from CompetitionTwoResutlsPage component
      ...
      axios.get(`${config.API_SERVER}/classroom/participant_detail/`)
        ...
        .then(res => {
          const selectedClients = res.filter(item => (item.cohort === currentCohort.pk))
                                     .filter(item => (!_.isEmpty(item.final_submission_2)))
          setClientsInCohort(clientsDetailsResultsForCompetitionTwo(selectedClients));
        })
    }
  ...
  const handleOrderParticipantsByResults = (e) => {
    ...
    else if(orderBy === "validation_accuracy") {
      if(!isAscendingOrder.validation)
        clientsOrdered = _.orderBy(clientsInCohort,['validation_accuracy'],['desc'])
      else  
        clientsOrdered = _.orderBy(clientsInCohort,['validation_accuracy'],['asc'])
      setIsAscendingOrder({...isAscendingOrder,validation:!isAscendingOrder.validation});
    }
...
export default CompetitionTwoResultsTable
  ...
  return (
    <>
      <StyledTableRow >
        ...
        <StyledTableCell align="center">  
          <span>{selectedClient.validation_accuracy}  </span>
        </StyledTableCell>
        <StyledTableCell align="center">  
      ...
      <TableRow >
        ...
        <Collapse in={openRow} timeout="auto" unmountOnExit>
          <Box margin={1}>
            <CompetitionTwoResultsAdminDisplay 
              selectedVisualization={selectedClient.model_type} 
